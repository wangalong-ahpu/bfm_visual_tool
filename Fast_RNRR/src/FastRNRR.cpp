...
QString MainWindow::fastRnrr(std::string& src_file,std::string& tar_file)
{
    Mesh src_mesh;
    Mesh tar_mesh;
    //std::string src_file;
    ...
    write_data(outpath.c_str(), src_mesh, scale);
    std::cout<< "write result to " << outpath << std::endl;
    delete reg;

    return QString::fromStdString(outpath);
}
